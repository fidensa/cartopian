...
| `marketing` | `marketing-claim` | `intended-outcome`, `when-to-apply`, `when-not-to-apply`, `principles-and-heuristics`, `working-process`, `decision-gates`, `failure-modes`, `evidence-and-verification`, `examples-and-counterexamples`, `stop-and-escalation`, `sources` |
| `operations` | `operations-change` | `intended-outcome`, `when-to-apply`, `when-not-to-apply`, `principles-and-heuristics`, `working-process`, `decision-gates`, `failure-modes`, `evidence-and-verification`, `examples-and-counterexamples`, `stop-and-escalation`, `sources` |
...
A pack revised to the operational mini-skill contract declares its **operational mini-skill sections** as its content areas — intended outcome, applicability and exclusions, conditional principles, working process, decision gates, failure modes, evidence, examples, stopping behavior, and sources — and carries its approved domain coverage as `domain_coverage`, judged by semantic review inside those sections. `software-delivery`, `research-inquiry`, `marketing-claim`, and `operations-change` are revised to that contract; `policy-governance` keeps its original topic areas until its own revision lands.
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `research-and-operations` | **recommended** | 5 | 173 | 44 | — | — | 32,768 |
| `research-and-software` | rejected as exemplar set | 4 | 181 | 46 | +8 | +2 | 32,768 |
| `software-and-operations` | rejected as exemplar set | 2 | 180 | 46 | +7 | +2 | 32,768 |
| `research-and-policy` | rejected as exemplar set | 3 | 174 | 44 | +1 | 0 | 20,480 |
| `research-and-marketing` | rejected as exemplar set | 3 | 180 | 46 | +7 | +2 | 32,768 |
| `marketing-and-operations` | rejected as exemplar set | 3 | 179 | 46 | +6 | +2 | 32,768 |
| `policy-and-operations` | rejected as exemplar set | 3 | 173 | 44 | 0 | 0 | 20,480 |
| `research-only` | rejected as exemplar set | 3 | 87 | 22 | −86 | −22 | 16,384 |
| `operations-only` | rejected as exemplar set | 2 | 86 | 22 | −87 | −22 | 16,384 |
| `software-only` | rejected as exemplar set | 1 | 94 | 24 | −79 | −20 | 16,384 |
| `research-operations-and-software` | rejected as exemplar set | 5 | 267 | 68 | +94 | +24 | 49,152 |
| `all-five-shapes` | **is the delivery scope** | 5 | 447 | 114 | +274 | +70 | 69,632 |
