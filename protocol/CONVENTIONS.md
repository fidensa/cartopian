...
A plan ref `KIND-NN-NNN` names its work kind, phase (`NN`), and a three-digit counter local to that kind within the phase. Supported work kinds are `BUILD`, `DESIGN`, `RESEARCH`, `TEST`, `RELEASE`, `VERIFY`, and `CORRECTIVE`.

Within a phase, each work kind has its own sequence starting at `001`: `BUILD-04-001`, `DESIGN-04-001`, and `TEST-04-001` may all coexist. An allocated counter is never reused within the same phase and kind. Task ids use a separate phase-wide sequence, so `TEST-04-001` may bind `TASK-04-003`; no numeric-suffix equality is implied or required. One plan ref binds one primary task, and every corrective task receives its own distinct plan ref — a ref is never reused merely because several tasks correct the same original item. A plan ref authored for a future phase that no task has bound yet is prospective planning content; revising its phase or kind-local sequence is normal plan evolution, not historical rewriting.
...
The corrected rule governs only work authored after activation: when the mediated task writer creates a task under the active contract, it records that creation in the project's append-only provenance log, and exactly those tasks are re-verified downstream. Every artifact that already exists remains valid, byte-stable, and accepted, with no migration, inventory, receipt, renumbering, rewrite, or reclassification.

Enforcement is mediated and shared. `cartopian write-task` refuses newly governed work whose plan ref is missing, malformed, unsupported in kind, phase-mismatched, or already bound to another task, or whose declared `Phase:` header is missing, malformed, or names a different phase than the task id and plan ref. `validate-task-readiness` and `task-bundle` report the same verdict through the `plan-ref-aligned` check and additionally verify the anchor chain for governed work — the declared phase file must exist and carry the task's plan ref. `cartopian plan-audit` reports invalid bindings and reused plan refs among newly governed work as blockers while reporting the activation-boundary state, never applying the corrected rule retroactively to artifacts that predate activation. CLI and MCP surfaces resolve through the one numbering-contract resolver, so their verdicts cannot drift.
...
Harness enforcement is agent-local and optional. For Claude Code, `cli/claude_stop_hook.py` is a **Stop** hook that refuses to end the turn while the report slot named by `CARTOPIAN_EXPECTED_REPORT_PATH` is absent or unparseable, feeding the assignee the instruction to finish in the foreground and publish. The shipped POSIX and PowerShell Claude wrappers add it through Claude's process-scoped `--settings` option on every dispatched handoff; they do not write a user, project, or local settings file, and Claude continues to load all three normal settings sources. The environment variable is the activation boundary: without it the wrapper adds no completion settings and the hook itself is inert. `CARTOPIAN_CLAUDE_BARE=true` continues to suppress auto-discovery, but the wrapper's explicit per-launch settings keep completion enforcement active. The hook delegates the completeness question to the same canonical observer the wait primitives use, imposes no timer of its own, and bounds itself to `CARTOPIAN_STOP_GUARD_MAX_BLOCKS` interventions (default 3) so a session that genuinely cannot report is never pinned open. It fails open on every error path. The completion hook is independent of the optional capability-refusal PreToolUse hook installed by `scripts/install.py --claude-hook <project-dir>`. Hosts with no comparable interception point rely on the prompt instruction alone.

Older projects may still contain a `claude_stop_hook.py` Stop entry written by an earlier installer. The wrapper reuses that exact entry in its per-launch settings so Claude's cross-scope array de-duplication runs it once, not twice. Re-running the explicitly requested `scripts/install.py --claude-hook <project-dir>` operation removes the obsolete completion entry while preserving unrelated settings and Stop hooks; ordinary install, update, project reconciliation, and dispatch never mutate registered projects for this migration.
