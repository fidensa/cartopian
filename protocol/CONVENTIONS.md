...
The machine vocabulary and field labels live once under `source_guidance` in `protocol/risk-and-practice-contract.json`. `cli/source_guidance.py` reads that authority and projects one deterministic record through `task-bundle`, `validate-task-readiness`, `handoff-packet`, `dispatch`, `parse-report`, `report-action`, `report-skeleton`, and `validate-report`. Because every CLI command is exposed through the shared MCP registry, the tool surface returns the identical record and diagnostics rather than reimplementing them.
...
Prompt volume is proportional to the assignment. A prompt carries the exact applicable inputs — the deidentified spec projection when a spec governs the work, the applicable report skeleton from `cartopian report-skeleton` rather than the full report template, and only the guidance bodies the selectors admitted for this task — never a full template, an unrelated guidance body, or a whole document the assignee needs one section of. A rework or correction prompt is a correction, not a re-assignment: it carries the specific findings or failed checks with their recovery, the affected excerpts, and the applicable skeleton, not the original assignment's full inputs again.
