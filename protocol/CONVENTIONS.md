...
- **Wrapper status is current, secondary evidence.** Automatic dispatch clears both the report and `<report-path>.status` after all preflights, publishes a fresh `state=running` status carrying the launch identity and expected variant before child creation, and removes that marker if supervisor creation fails. The wrapper may write `state=exited`; the outer supervisor atomically publishes the final clean/error/timeout result as a fallback so a custom wrapper or wrapper-launch failure cannot strand `running`. Retained-log truncation is nonterminal metadata and never changes that lifecycle result. Manual launches may omit running identity; absence remains valid and waits degrade to report-only observation. A stale or variant-mismatched status cannot terminate a new handoff. The `.status` file remains transient and is removed through `cartopian delete-report`. Both wait commands are read-only — they never write project state, move tasks, launch processes, or read `.launch.log` bodies.
