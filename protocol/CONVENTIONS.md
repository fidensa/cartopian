...
`cli/output_safety.py` is the runtime source of truth for the shipped output-safety defaults. Every prompt written through `cartopian write-prompt`, and every legacy prompt immediately before automated dispatch, receives one generated operational rule: each individual command returns at most **200 lines or 32 KiB**, whichever comes first; broad diagnostics go to a temporary file and only a bounded excerpt plus outcome returns to the assignee. This starting command budget was exercised by the successful corrected retry. The original 13.7 MB / 291,000-line failure and the later 1,314,114-byte / 29,744-line rework are the evidence that prompt-only discipline is insufficient.

Automated dispatch additionally launches every configured wrapper through one standard-library supervisor. The shipped cumulative defaults are **2,000 lines / 256 KiB** for the wrapper stream and, independently, **400 lines / 64 KiB** for retained `<report-path>.launch.log` diagnostics. Operators may set positive-integer `CARTOPIAN_STREAM_LINE_LIMIT`, `CARTOPIAN_STREAM_BYTE_LIMIT`, `CARTOPIAN_LOG_LINE_LIMIT`, and `CARTOPIAN_LOG_BYTE_LIMIT` overrides in the dispatch environment; malformed, zero, or negative values refuse before launch. Dispatch normalizes and re-exports those values to every wrapper, so Codex, Claude, Gemini, Devin, custom configured agents, POSIX wrappers, and native PowerShell/CMD routing all pass through the same boundary.

Accounting is over raw emitted bytes. Line count is the number of LF bytes plus one when the observed stream ends in a non-empty trailing fragment: CRLF counts once, a final LF adds no empty line, and multibyte or invalid UTF-8 bytes receive no special treatment. Crossing either cumulative stream ceiling stops further observation, terminates or forcibly contains the wrapper process tree, exits `125`, and atomically publishes `classification=output-overflow` plus configured limits, observed byte/line counts, retained path/size, termination result, report presence, and guarantee scope in the bounded status file. A complete acceptable report already authoritative before the overflow boundary wins; a report appearing afterward does not relabel overflow.

Launch-log retention is independently capped and constant-memory with respect to child output. It retains a bounded diagnostic prefix and writes an explicit truncation marker when the log cap or stream cap omits bytes. Unsafe, unwritable, symlinked, hard-linked, or non-regular destinations degrade to unavailable retention; they never redirect output into the PM-visible stream. The wait commands consume report/status metadata only and never open, read, summarize, or use launch-log contents as progress evidence. `delete-report` removes both status and log companions on slot clear and close.

The guarantee scope is intentionally **`observable-wrapper-stream`** in the host runtime. It is not pre-model interception and does not prove that bytes were excluded from a provider's private model context. The bounded log sink is retention, not that proof. Dispatch and overflow records expose `pre_model_ingestion_guaranteed=false` and `provider_private_context_exclusion_guaranteed=false` so callers cannot silently strengthen the claim.
...
- **The report file is the authoritative completion signal and publication boundary.** Path appearance alone is not terminal. A complete report that parses as the expected handoff variant is terminal and routes its actual verdict (`accepted`, `blocked`, `failed`, `changes-requested`, or `rejected`). Incomplete or temporarily malformed bytes remain nonterminal while the current wrapper can still finish publication. After wrapper exit, stable malformed bytes classify `failed-to-parse`; a non-zero exit with no report classifies `failed`; a clean exit with no report classifies `exited-without-report`. A `timeout`, hard process stop, crash, or missing/late/permanently invalid report is not successful completion evidence.
- **Wrapper status is current, secondary evidence.** Automatic dispatch clears both the report and `<report-path>.status` after all preflights, publishes a fresh `state=running` status carrying the launch identity and expected variant before child creation, and removes that marker if supervisor creation fails. The wrapper may write `state=exited`; the outer supervisor atomically publishes the final clean/error/timeout result as a fallback so a custom wrapper or wrapper-launch failure cannot strand `running`. On stream overflow it publishes the distinct `output-overflow` status and bounded counters after containing the wrapper. Manual launches may omit running identity; absence remains valid and waits degrade to report-only observation. A stale or variant-mismatched status cannot terminate a new handoff. The `.status` file remains transient and is removed through `cartopian delete-report`. Both wait commands are read-only — they never write project state, move tasks, launch processes, or read `.launch.log` bodies.
- **The task report slot is shared safely between coder and reviewer.** Before clearing an accepted coder report for task-closure review, the review-prompt writer captures its full bounded content, outcome facts, source identity, and SHA-256 content identity inside the generated review context. That current binding also covers exact operator evidence, PM-derived artifact paths, task, prompt, and review target. Review preflight recomputes from the captured snapshot after the coder filename is empty; it never requires the transient coder report to remain. Task review expects the `review` report variant, so reintroduced coder-shaped content cannot satisfy reviewer completion.
- **Waiting is terminal by default: one launch, one wait call, one result.** Called without `--max-block`, a wait primitive blocks until a terminal observation, bounded by the resolved handoff timeout as the absolute ceiling. This is the only supported shape. Cartopian has no wake, resume, or callback mechanism, and no host is assumed to supply one — a blocking call that survives to the report is the entire completion mechanism.
