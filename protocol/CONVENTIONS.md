...
- **Wrapper status is current, secondary evidence.** Automatic dispatch clears both the report and `<report-path>.status` after all preflights, publishes a fresh `state=running` status carrying the launch identity and expected variant before child creation, and removes that marker if child creation fails. The wrapper atomically replaces it with `state=exited`, exit facts, the same launch identity, and expected variant. Manual launches may omit running identity; absence remains valid and waits degrade to report-only observation. A stale or variant-mismatched status cannot terminate a new handoff. The `.status` file remains transient and is removed through `cartopian delete-report`. Both wait commands are read-only — they never write project state, move tasks, or launch processes.
