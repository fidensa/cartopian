...
- **The host's tools/call ceiling is a hard constraint, and it is checked before launch.** Every MCP host caps a single `tools/call`, by wall clock, by silence between messages, or both, and on some hosts that cap is *shorter than the protocol's default 60m role timeout*. When it is, the wait dies mid-handoff while the assignee keeps running unobserved. `cartopian dispatch` therefore refuses to launch when `roles.<role>.timeout` exceeds the resolved host budget, naming both the mismatch and its remedies; `cartopian host-capability` reports that budget on demand, and both wait primitives echo it as `host_wait_budget`. The budget is resolved from evidence — the connected host's `clientInfo` and its own config — and a host that is not recognized resolves to *unknown*, which fails the gate rather than inheriting another host's numbers. The three honest responses to a refusal are to raise the host's ceiling, to lower the role timeout to fit it, or to dispatch that role manually and monitor the report path. Inventing a periodic status-check rhythm, or assuming a long blocking call will survive a host that has not been shown to allow one, is not among them. Host-by-host ceilings, defaults, and the exact configuration keys live in `skills/register-mcp.md`.
