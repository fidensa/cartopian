...
- **Tool-owned deterministic filesystem actions (do it yourself):** for an entry that declares a registered filesystem migration, run:
  ...
  The shipped registry (`cli.migrations.ENTRY_VERSIONS`) names exactly which entries carry a deterministic filesystem action. Run the command once for each registered entry when that entry applies. Do not invoke it for entries outside the registry — those contain only config, operator-choice, or judgment-dependent PM steps (v0.11.0, v0.12.0, and v0.13.0 are three: the v0.11.0 standards re-sort goes through `write-standards`, the v0.12.0 delivery contract goes through `write-plan`, and the v0.13.0 run-boundary rename is wholly owned by `migrate-config` — never `apply-migration-entry`). The v0.13.0 plan may also list a `materialize-registered-project` step: the retired pace pair was resolved against the shared operator-global file, so where the preferred contract cannot state one shared value that preserves every registered project, `migrate-config` writes the affected project's own preserved boundary and budget into that project's `cartopian.toml` before retiring the shared source. Read the planned steps and tell the operator which other registered projects a run will touch; those writes change no other field and never advance another project's schema marker.
