...
| **Start Session** | `start-session.md` | Resolve the current project, read `STATE.md`, and proceed with the next sequential PM action per the linear execution default | Continues into `run-task.md` without a confirmation prompt; stops for blockers, plan-level forks, and operator-reserved decisions |
