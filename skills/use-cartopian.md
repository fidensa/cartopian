...
- On HTTP 200, compare `tag_name` to the installed version.
  ...
  - If they differ, tell the operator a newer release is available (`<installed>` → `<latest>`) and ask, **once**, whether to upgrade now (or continue / decide later). If yes, read `cartopian://skills/check_for_updates` and follow it to perform the upgrade. **Carry forward that the operator already approved upgrading to the latest release, so the runbook skips its own upgrade confirmation (Step 5) and proceeds directly** — do not make the operator answer the same question twice. Resume here when it returns. If no or "later", proceed to Step 1.
