...
- The exact `## Practice-pack result` returned in Stage 1. Include a body only for `selected`; `none` contributes no body. Never issue a handoff from an `ambiguous` or `invalid` result. Do not use selection to change the risk result, activate judgment guidance, alter configured review policy, or require review.
