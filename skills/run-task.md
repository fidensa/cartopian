...
The command resolves the allowlisted `prompts/` destination from the `--prompt-id`, so the PM never supplies a free-form path; re-issuing it overwrites the same prompt in place on a retry. The assignee handoff is **deidentified**: name the work by its title and address every resource by file path. Do **not** put project-management identifiers (the task id, plan ref, spec id, `FR-`/`NF-` requirement refs, decision refs) anywhere in the prompt body — they map to nothing once PM data is archived and can leak into the delivered work. The prompt is also **audience-scoped**: include a statement only when it changes how the assignee performs, evidences, bounds, or reports the work. Downstream lifecycle facts the packet resolves — the configured review role, review launch policy, what the PM does after the report lands — stay out of the prompt body: they change nothing the assignee does and invite hedging, such as deferring completeness to a reviewer or reporting work not ready pending a review that has not been assigned. The pasted risk, judgment, practice-pack, and source-guidance results are the only carriers of lifecycle-adjacent expectations, and they are never paraphrased into additional prose. The prompt body must be directed at the assignee and include, sourced from the `handoff-packet` record:

- A role preface as the first prose section (`## Your role`): address the assignee from the record's `role_description` — who they are and what the role does for this assignment. Orientation only; it grants no authority beyond the role's configured grants and carries no PM identifiers.
...
- The assignment launch boundary from `templates/PROMPT.md`: process cwd is the selected Cartopian project root; product work occurs through the declared work roots; work-root access does not grant PM lifecycle authority. State the shipped wrapper difference when it applies — Codex, Claude, and Antigravity can widen to declared paths, while an active Devin sandbox cannot and may leave those paths unwritable.
...
- When the current-cycle `STANDARDS.md` declares working standards, constraints, or quality checks that bind this assignment (style and formatting conventions, required development practices, mandated validation tooling), the applicable excerpt in a `## Project standards` section — deidentified, and never the whole document or planning-side metadata (stack rationale, cycle constraints, open standards questions) the assignee cannot act on. Omit the section when none apply.
...
- When `existing_deliverable_input.required` is true, an `## Existing deliverable input` section containing the complete current UTF-8 content of that project resource. The role cannot read governance resources directly, so a path or summary is insufficient. After `write-prompt`, rerun `handoff-packet` and require `existing_deliverable_input.ok: true`; both manual and automatic handoff paths fail closed before launch when the content is absent or stale.
- For each `dependency_deliverable_inputs` record with `required: true` and reason `dependency-deliverable-is-not-role-readable`, an `## Upstream contract input` section containing that dependency deliverable's complete current UTF-8 content — the upstream contract the assignee builds on must be readable, never inferred. A `dependency-deliverable-missing` record is a blocker: persist the upstream deliverable with `cartopian write-resource` before authoring the prompt. Rerun `handoff-packet` after `write-prompt` and require every record's `ok: true`.
- Absolute expected report path (from the record's `expected_report_path`).
- The report skeleton: run `cartopian report-skeleton <task-path>` and paste the returned `skeleton` into the prompt as the report shape the assignee fills in — **instead of** the full report template. The skeleton carries only this task's applicable sections with machine-owned values (work-root names, prefilled assignee-facing source-evidence rows) already filled; the assignee supplies substantive evidence only.
...
- The exact `## Judgment guidance` result returned in Stage 1, including its `active_cards`, `ordered_activation_reasons`, `inactive_cards`, `guidance_identity`, `loaded_guidance_bytes`, `guidance_budget_bytes`, and `context_receipt`. Include a body only for `active`, and then exactly the one returned central body — never a per-card copy, a second grammar, or an anti-rationalization table of your own. Four active cards carry the same one body as one active card. `none` contributes no body. Never issue a handoff from an `invalid` result. Do not use activation to change the risk result, select a practice pack, alter configured review policy, or require review.
- The exact `## Practice-pack result` returned in Stage 1, including its `body_identity`, `loaded_body_bytes`, `body_budget_bytes`, `applicable_sources`, and `context_receipt`. Include a body only for `selected`, and then exactly the returned body — never a second pack body, the metadata catalog, a full source document, structural-exemplar text, or watchlist content. `none` contributes no body. Never issue a handoff from an `ambiguous` or `invalid` result. Do not use selection to change the risk result, activate judgment guidance, alter configured review policy, or require review.
- When `source_guidance.outcome = valid`, the record's `deidentified_guidance` pasted into `## Source guidance` exactly, plus a requirement that the completion report provide `## Source evidence` naming the non-empty subset of supplied source identities and applicable contexts actually used. Explicitly tell the assignee to omit unused guidance sources and never introduce a source outside the supplied guidance. `invalid` is a blocker and `not-applicable` / `not-declared` adds no source-guidance section. Do not paste raw PM identifiers from the owning task or spec.
