...
- **`blockers`**: any non-empty `blockers` array is a PM-level blocker (e.g. `no active phase detected but tasks are present`, `unresolved open question in STATE.md: …`). Surface each entry to the operator and stop. Do not proceed to Stage 3 while blockers exist. A `config-schema migration required` blocker (the project's `[project].protocol_version` is behind the shipped protocol) is PM-owned remediation: surface it and, on the operator's approval, run `migrate project` (`skills/migrate-project.md`), which brings the marker current via `cartopian update-config`. Do not tell the operator to hand-edit `cartopian.toml`.
