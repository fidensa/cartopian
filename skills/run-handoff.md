...
Automated dispatch also places the configured wrapper under the common bounded-output supervisor. The centrally generated prompt rule limits each returned command to 200 lines / 32 KiB; cumulative wrapper-stream and retained-log limits come from the normalized `cli/output_safety.py` contract and are enforced outside the assignee. `output-overflow` is terminal and distinct from ordinary failure or timeout. Surface its bounded status metadata only: configured limits, observed byte/line counts, retained size/path, termination result, report presence, and `guarantee_scope=observable-wrapper-stream`. Never open or summarize `<report-path>.launch.log` while waiting. This scope is host-runtime observation after the wrapper emits bytes; it is not proof of pre-model interception or provider-private-context exclusion.
