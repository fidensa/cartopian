...
- `existing_deliverable_input` — the preflight for a project-resource deliverable that already exists. When `required: true`, the role lacks `read:governance`, so the prompt must carry the complete current UTF-8 resource text as a machine-created typed input payload in its `## Existing deliverable input` section. Do not paste the content by hand: `compose-assignment-prompt` and the mediated `write-prompt` writer materialize the payload block (marker `cartopian-input`, bound by logical resource, byte count, and SHA-256), and a hand-authored declaration is refused. After writing the prompt, rerun `handoff-packet` and require `ok: true`; manual handoff and `dispatch` both fail closed when the bound payload is missing, mutated, or duplicated (the record's `prompt_payload` state token names which). `content_sha256` and `content_bytes` identify the resource version being checked. This does not remove or narrow a role's configured grants.
- `dependency_deliverable_inputs` — the same preflight applied to each `Blocked by:` dependency's project-resource deliverable. A task that builds on an upstream contract must be able to read it: a record with `required: true` and `reason: dependency-deliverable-is-not-role-readable` means the role lacks `read:governance` — the writer materializes that dependency's complete current resource text as a typed payload in the `## Upstream contract input` section; rerun `handoff-packet` and require every record's `ok: true`. A `dependency-deliverable-missing` record means the upstream deliverable was never persisted; persist it with `cartopian write-resource` before any dispatch — the assignee must never be left to infer an upstream interface.
- `input_payload_audit` — the whole-prompt payload audit. Every payload declaration in the prompt must be machine-resolved, current, and unique; `problems` names any declaration that is hand-authored, bound to a resource that is not an assignment input, stale against the resource on disk, malformed, or duplicated. Both manual handoff and `dispatch` fail closed on a non-empty `problems` list.
...
7. When `existing_deliverable_input.required` is true, the prompt must carry the complete current resource text as a machine-created typed input payload in `## Existing deliverable input`. Never paste the content by hand, summarize it, or direct the assignee to its governance-scoped path: write the prompt through `write-prompt` (or `write-prompt --composed-file`) and the writer materializes the payload block from the machine-resolved input — a hand-authored payload declaration is refused. Rerun `handoff-packet` after writing and require `existing_deliverable_input.ok: true`. The same applies to each `dependency_deliverable_inputs` record with `required: true`: the writer materializes that dependency's payload into `## Upstream contract input`; rerun until every record's `ok` is `true` and `input_payload_audit.ok` is `true`.
...
The launched wrapper sets its single `CARTOPIAN_TIMEOUT` deadline from the resolved role launch timeout, with the protocol default of `60m` when that value is unset. It enforces the deadline at the OS level (`timeout`/`gtimeout` on POSIX, `Start-Process` + `WaitForExit` on PowerShell) and exits with exit `124` when the deadline elapses. If `timeout`/`gtimeout` is absent, the Antigravity bash wrapper alone remains bounded through its aligned internal `--print-timeout` and maps that expiry onto the same exit-`124` status contract; other bash wrappers warn and run unbounded. Per FR-012 launch semantics, assignee CLIs run with cwd set to the cartopian project root (the registered project path). `dispatch` exports declared work-root absolute paths in canonical resolved order through `CARTOPIAN_WORK_ROOTS`; no export is present when the project declares none. The Codex wrapper widens `workspace-write` with those paths, and the Claude and Antigravity wrappers pass each through `--add-dir`. The Devin sandbox has no per-path widening surface, so its wrapper warns that declared work roots may be unwritable while that sandbox is active. Custom agents must state and honor their own equivalent behavior.
