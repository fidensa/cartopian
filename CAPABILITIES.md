...
**Not a capability boundary.** The Claude wrappers load the completion adapter `cli/claude_stop_hook.py` through process-scoped `--settings` only when dispatch exports `CARTOPIAN_EXPECTED_REPORT_PATH`. This **Stop** hook refuses a report-less end-of-turn (see `protocol/CONVENTIONS.md` § Foreground Completion), grants and denies nothing, reads no capability configuration, and is not part of either boundary above or of the containment matrix. It enforces handoff *completion discipline* and fails open by design, where the separately installed refusal adapter fails closed. `scripts/install.py --claude-hook` remains the explicit project-level capability-hook operation; it does not register completion enforcement.
