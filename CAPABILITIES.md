...
**Not a capability boundary.** `scripts/install.py --claude-hook` registers a second hook in the same settings file — the completion adapter `cli/claude_stop_hook.py`, a **Stop** hook that refuses a report-less end-of-turn (see `protocol/CONVENTIONS.md` § Foreground Completion). It grants and denies nothing, reads no capability configuration, and is not part of either boundary above or of the containment matrix; it enforces handoff *completion discipline* and fails open by design, where the refusal adapter fails closed.
