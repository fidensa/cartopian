...
**Honest tiering.** The read boundary is *enforced* only on a host whose interception point actually intercepts the read tools — for Claude Code, only when the registered PreToolUse matcher covers them (a pre-read-boundary, write-only registration does not; any install or update run replaces it in place, since project hook registration is a required install surface). On any host without read interception, the read boundary is **advisory + detection**, never claimed as enforced; the detection floor remains the fail-safe. `cartopian containment-matrix <project-path>` renders the per-host tier for each boundary from real evidence and disclosures the residuals plainly.

**Not a capability boundary.** The same required install surface registers a second hook in the same settings file — the completion adapter `cli/claude_stop_hook.py`, a **Stop** hook that refuses a report-less end-of-turn (see `protocol/CONVENTIONS.md` § Foreground Completion). It grants and denies nothing, reads no capability configuration, and is not part of either boundary above or of the containment matrix; it enforces handoff *completion discipline* and fails open by design, where the refusal adapter fails closed.
