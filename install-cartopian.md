...
**Output:** Cartopian copied to the install root (default: `~/.cartopian/` on macOS / Linux / WSL, `%USERPROFILE%\.cartopian\` on native Windows — or wherever the operator's `--prefix` points), `bin/` and the platform-appropriate wrapper directory (`wrappers/bin` on Unix, `wrappers\ps1` on Windows) added to the user PATH, `cartopian --help` exits 0 on every supported platform (via the shipped `bin/cartopian.cmd` shim on native Windows), the agent wrappers (`cartopian-codex`, `cartopian-claude`, `cartopian-devin`, `cartopian-agy`, `cartopian-opencode`, `cartopian-hermes`) resolve as bare commands, the Cartopian MCP server is registered with the operator's agent so they can say "use cartopian" from any directory, and `VERSION` at the install root records the installed git ref.
...
- `--intake-hooks` — after the install/update, writes Cartopian's request-evidence intake hooks at user level for each detected host (`~/.claude/settings.json` for Claude Code, `~/.codex/hooks.json` for Codex CLI and desktop, `~/.gemini/config/hooks.json` for Antigravity, a plugin under `~/.hermes/plugins/` for Hermes, and a plugin under `~/.config/opencode/plugins/` for opencode). Idempotent; unrelated hooks and settings are preserved. Ask the operator before adding it: it records, for every new interactive session on those hosts, the operator's prompt text, the assistant's final message per turn, session and turn identifiers, and the working directory under `<install_root>/intake/`. Sessions that never select a Cartopian project keep at most 200 events / 2 MiB and are discarded when they end; dispatched Cartopian sessions are never recorded. The installer prints the same disclosure. Hooks load at session start, so the operator must open a fresh session on each host afterwards. Codex additionally skips the hooks until the operator trusts them in `/hooks` (trust is keyed to the exact hook definition and must be repeated after a re-install that changes it).
- `--remove-intake-hooks` — standalone rollback that removes only Cartopian's intake hook entries and shims from those locations and touches nothing else.
...
`register-mcp` detects which supported agents are present on the machine, shows which are already registered, and applies the appropriate recipe for each agent the operator selects. For Claude Code, Codex, Antigravity, Devin, Windsurf, opencode, and Hermes (Hermes remains experimental pending native Windows acceptance; its macOS handoff and Slack direct-message path have passed) it does the full two-part hookup — registers the MCP server **and** installs a "use cartopian" trigger bridge (skill or command) so the entry phrase reads the authoritative `cartopian://skills/use_cartopian` resource. Claude Desktop and Cursor are MCP-only (no local bridge mechanism). Unsupported client identifiers are refused; there is no caller-defined executable, configuration path, bridge destination, or generic registration fallback.

Do not stop at registration. That skill's Stage 4 raises each host's tool-call ceiling, which is what makes handoffs work at all: Cartopian waits for an assignee by holding one `tools/call` open for up to `roles.<role>.timeout` (default `60m`), and Codex caps that call **below** the default out of the box (opencode's unconfigured default is a 60-second *idle* window, though Cartopian's registration recipe writes a raised `timeout` alongside the entry and its blocking waits heartbeat through it; Hermes's unconfigured default is a 300-second hard wall clock per call, so its registration writes `timeout: 3900` to clear the default role timeout in one terminal wait). Antigravity's ceiling is a fixed 180-second wall clock per call that **cannot be raised** — no per-server timeout key, no environment override, and progress does not extend it. Automatic dispatch therefore requires the role timeout to fit below that ceiling; longer roles must be launched manually and then observed with `max_block` slices below the ceiling. Stage 4 documents this rather than writing config. Without Stage 4, `cartopian dispatch` fails closed on hosts whose wait cannot survive.
...
- **Entry point**: tell the operator, in plain language, how to enter Cartopian PM mode from each agent they configured — say "use cartopian" in Claude Code, Codex, Devin for Terminal, and Hermes (or type `/use-cartopian` or `$use-cartopian` in Codex, Antigravity, Windsurf, and opencode). After any required client restart, the installed bridge reads the `use_cartopian` resource and begins registry-first project selection. MCP-only clients use the `use_cartopian` prompt from their prompt picker.
...
Behavioral contract changes activate only when the release carrying them is installed **and** any affected running process is proven fresh — written source or installed files are never proof that a running MCP server changed (see `protocol/INSTALL_VERIFICATION.md`). A concrete example is the split-report contract: releases before it use one shared task-scoped report slot for coder and reviewer completion, while releases carrying it preserve the coder report at `reports/REPORT-NN-NNN.md` and publish task-review completion to the independent `reports/REPORT-NN-NNN-review.md`. A session served by a pre-split running process continues emitting and expecting the shared path; do not claim split-report behavior — or reference the future review-report path — until the corrected release is installed and the running server's restart state supports the activation claim. Review prompts generated under the old embedded-evidence contract are refused by the new runtime with deterministic regenerate guidance.

The plan/task numbering contract follows the same double boundary. Releases carrying it enforce kind-first plan refs with one phase-wide sequence and an unchanged task-scoped suffix (`protocol/CONVENTIONS.md` § Plan/Task Numbering Contract) only for chains newly authored after the reviewed correction is carried by an operator-owned release tag, installed, and proven active — reviewed or copied source activates nothing, installed files alone activate nothing for a running MCP server without fresh-process proof, and no project migration, schema-marker advance, or inventory of existing artifacts is involved. A stale client keeps the historical behavior until the running process proves it serves the corrected installed content, and every pre-activation chain remains valid unchanged forever, exactly as written.
