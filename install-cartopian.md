...
The plan/task numbering contract follows the same double boundary. Releases carrying it enforce kind-first plan refs with kind-local counters (`protocol/CONVENTIONS.md` § Plan/Task Numbering Contract) only for plan/task pairs newly authored after the reviewed correction is carried by an operator-owned release tag, installed, and proven active — reviewed or copied source activates nothing, installed files alone activate nothing for a running MCP server without fresh-process proof, and no project migration, schema-marker advance, or inventory of existing artifacts is involved. Task ids keep a separate phase-wide sequence and do not share counters with plan refs. A stale client keeps the historical behavior until the running process proves it serves the corrected installed content, and every pre-activation plan/task pair remains valid unchanged forever, exactly as written.
