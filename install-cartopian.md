...
The plan/task suffix-alignment contract follows the same double boundary. Releases carrying it enforce the phase-wide numbering rule (`protocol/CONVENTIONS.md` § Plan/Task Numbering Contract) only for plan/task pairs newly authored after the reviewed correction is carried by an operator-owned release tag, installed, and proven active — reviewed or copied source activates nothing, installed files alone activate nothing for a running MCP server without fresh-process proof, and no project migration, schema-marker advance, or inventory of existing artifacts is involved. A stale client keeps the historical kind-local numbering until the running process proves it serves the corrected installed content, and every pre-activation plan/task pair remains valid unchanged forever, exactly as written.
