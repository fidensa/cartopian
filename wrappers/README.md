...
- **macOS only:** GNU coreutils provides the `gtimeout` binary the bash wrappers use to enforce `CARTOPIAN_TIMEOUT` at the OS level:
  ...
  ```
  Without coreutils, most bash wrappers warn at launch and run unbounded — handoffs still execute, but a hung assignee can run forever instead of being killed at the configured deadline. `cartopian-agy` is the exception: its aligned `--print-timeout` remains active and an internal expiry is reported through the normal exit-`124` timeout contract. Linux distributions ship `timeout` in coreutils by default; native Windows uses the shared PowerShell `ProcessStartInfo` supervisor and needs no extra install.
...
The bash wrappers run `timeout <duration> <real-cli> ...` so the OS owns the deadline; the upstream process receives SIGTERM at the configured wall-clock limit (exit code 124). The PowerShell wrappers achieve the same with a direct `ProcessStartInfo` child plus the same single deadline; npm `.cmd` shims are bridged through a no-profile PowerShell child with argv carried as JSON/base64 over stdin, and timeout/report completion reaps the full descendant tree. That general bridge does not authorize an underlying Claude `.cmd`/`.bat` shim for a hook-bound launch: the Claude wrapper refuses it because the required settings argv boundary must terminate at one exact native executable. The PM does not poll or watchdog the running process — it dispatches and waits for the platform's background-completion signal.
...
| `CARTOPIAN_CLAUDE_EXECUTABLE` | _(set by dispatch)_ | Exact absolute Claude executable selected during shared preflight. Hook-enabled wrappers require and launch this path directly; they do not resolve Claude through `PATH` again. On native Windows the path must name a native executable, not a `.cmd`/`.bat` shim. |
| `CARTOPIAN_CLAUDE_BARE` | `false` | Skip auto-discovered plugins and hooks (`true`/`false`) only on launches that need no Cartopian hook. A hook-enabled dispatch refuses `true`, because current Claude releases also suppress explicit process-scoped hooks in bare mode. |
...
For an activated native macOS/Linux handoff, that object also enables Claude's OS sandbox with fail-closed startup and no unsandboxed retry. It denies shell and child-process writes to the Cartopian project root, its checkout `.git` marker, and its resolved Git directory/common directory, including linked-worktree metadata outside that root. Declared work roots are added to `allowWrite` only for a role holding `write:worktree`; otherwise they are added to `denyWrite`. Because a path-only PreToolUse allow cannot atomically bind Claude's later macOS open, activated structured mutation tools deliberately refuse external work roots even when that grant is held; authorized work-root mutation uses Bash, where the OS sandbox decides at the filesystem operation. Git metadata inside an authorized external product work root remains writable with that root. Protected paths, authorized work roots, and Claude's fixed/per-UID temp, npm-log, and debug write exceptions must be disjoint in both directions; nested writable roots and any overlap that could let an ancestor rename move a protected inode refuse in shared dispatch/rehearsal preflight. Pre-existing cross-boundary hard links also refuse, and Linux mount-coordinate validation rejects nested/bind-mount aliases. A policy path containing `*`, `?`, `[` or `]`, or any control character, also refuses because Claude/Git could treat it as syntax, skip it, or split it rather than enforce the literal path. Activated WSL2 handoffs refuse pending seccomp attestation. Activated native-Windows handoffs refuse before Claude until Cartopian can attest both a shell sandbox and an exact native Claude executable chain; Windows is not a separately accepted partial-containment path.

Every activated capability launch also passes `--strict-mcp-config` without an explicit MCP config. This removes user-, project-, and plugin-provided MCP tools whose `mcp__...` events are outside the built-in-tool matcher and whose servers run outside Claude's shell sandbox. It also disallows delegated-agent/worktree tools, team lifecycle tools, durable cron tools, peer-session message/file routing, and remote agent triggers (`Agent`/legacy `Task`, `EnterWorktree`, `ExitWorktree`, `TeamCreate`, `TeamDelete`, `CronCreate`, `CronDelete`, `CronList`, `SendMessage`, `SendFile`, and `RemoteTrigger`); the refusal hook independently denies any such call that still reaches PreToolUse. Those tools can move or defer execution, route mutations through another session, or mutate shared Git metadata outside the captured project/work-root boundary. Normal user/project/local hooks and plugins are excluded with their settings sources; administrator-managed extensions remain operator-controlled trusted policy.

The wrappers never write user, project, or local Claude settings. Completion-only and ungated launches retain all normal settings sources; because the Stop hook is bounded and fail-open completion discipline, those sources remain hot-loadable in that case. An activated capability launch instead passes `--setting-sources ""`. This excludes normal user, project, and local settings—and therefore their hooks, plugins, `env`, permissions, and sandbox additions—for the entire process, so a cwd change into an authorized work root cannot hot-load an exception. Activated process settings also set `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`, preventing the session from persisting instructions under `~/.claude/projects`.

Claude still loads the top-level `env` from the legacy global config (`~/.claude.json`, or `.claude.json` under an absolute shell-level `CLAUDE_CONFIG_DIR`) even with an empty source list; its other fields are inert on this path. Cartopian therefore validates the inherited environment plus that effective legacy-global environment before launch. It refuses hook-subprocess loader/startup controls (`PYTHON*`, `LD_*`, `DYLD_*`, `OPENSSL_*`, `BUN_*`, `NODE_OPTIONS`, `NODE_PATH`, `__PYVENV_LAUNCHER__`, `GLIBC_TUNABLES`, `GCONV_PATH`), config and settings-supplied temp redirection, `CLAUDE_CODE_TMPDIR`, `CLAUDE_TMPDIR`, the `CLAUDE_CODE_PROCESS_WRAPPER` environment control, hook-suppressing modes, effective external-ripgrep selection, and a non-empty effective `RIPGREP_CONFIG_PATH`. For an activated launch, dispatch replaces inherited `TMPDIR`/`TMP`/`TEMP` with a direct user-owned mode-0700 `~/.cartopian/claude-host-tmp` before the Claude version probe, so the unsandboxed SRT mux socket is protected; Claude 2.1.278 gives sandboxed Bash a separate writable per-user temp. Dispatch also strips Bash/function/tracing, Bun/Node, loader, Python, and OpenSSL startup controls before its `-I -S` output supervisor and privileged-startup Bash wrapper execute, and every pre-containment Git/version probe receives an explicit sanitized environment. On macOS it protects the selected `rg` and every earlier search directory. Linux similarly protects `rg`, `bwrap`, and `socat`; any protected search path intersecting an authorized writable work root refuses launch. The structured hook plus shell policy protect the legacy file, the exact bound Python and Claude executables, Cartopian enforcement/configuration roots, and host-helper search paths. Its arguments capture the canonical Cartopian config root and each work root's path/device/inode; the hook verifies those identities on every call and resolves target spellings before lexical `..` collapse. The activated Stop hook stores its bounded counter in a direct private directory below the protected config root rather than inherited temp. Administrator-managed sandbox, hook-disabling, process-wrapper, or environment settings remain trusted policy overrides and reported containment residuals. Each hook uses Claude's shell-free exec form with the bound interpreter in `-I -S` isolated mode, with the installed script, dispatched identity, and protected paths as separate arguments.

Completion-only hook launches require Claude Code 2.1.139 or newer for shell-free hook `command`/`args`. They may run on native Windows only when dispatch binds a direct native Claude executable; an underlying `.cmd` or `.bat` shim is refused even though the general PowerShell supervisor can bridge batch shims for non-hooked invocations. Every activated capability launch requires 2.1.278 or newer for strict MCP and empty-source isolation, and activated native-Windows launches are refused at the unattested host boundary described above. Dispatch resolves and pins the exact Claude and Python executables; the wrapper runs containment preflight with that Python, probes that Claude's version, and launches that same Claude path. It refuses a missing, relative, non-executable, outdated, or unidentifiable binding before launch.

`CARTOPIAN_CLAUDE_BARE=true` remains available for launches that need no Cartopian hook. Capability- or completion-hook launches refuse it before Claude starts because current Claude releases suppress explicit `--settings` hooks in bare mode. The same boundary rejects inherited Claude startup modes such as `CLAUDE_CODE_SIMPLE` or `CLAUDE_CODE_SAFE_MODE` that can suppress or preempt the required hooks.

Older Cartopian versions optionally wrote capability and completion entries into project `.claude/settings.json`. Activated dispatch excludes that normal settings file, so an old registration neither runs twice nor requires cleanup before an activated handoff. Completion-only and ungated launches still load normal settings; shared preflight refuses a persistent Stop registration that would collide with the report-bound process hook. Operators may remove obsolete entries with the retained compatibility command:
