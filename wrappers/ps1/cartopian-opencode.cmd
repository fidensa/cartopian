@echo off
rem Native-Windows PATH shim for the cartopian-opencode assignee wrapper.
rem
...
rem `.ps1` directly. This `.cmd` (in PATHEXT) is what makes `cartopian-opencode`
...
where /q pwsh
if not errorlevel 1 (
    pwsh -NoProfile -ExecutionPolicy Bypass -File "%~dp0cartopian-opencode.ps1" %*
    exit /b %ERRORLEVEL%
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0cartopian-opencode.ps1" %*
exit /b %ERRORLEVEL%
