@echo off
rem Native-Windows PATH shim for the cartopian-agy assignee wrapper.
rem
...
rem `.ps1` directly. This `.cmd` (in PATHEXT) is what makes `cartopian-agy`
...
where /q pwsh
if not errorlevel 1 (
    pwsh -NoProfile -ExecutionPolicy Bypass -File "%~dp0cartopian-agy.ps1" %*
    exit /b %ERRORLEVEL%
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0cartopian-agy.ps1" %*
exit /b %ERRORLEVEL%
