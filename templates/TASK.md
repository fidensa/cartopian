...
- `project:resources/relative/path` — the work product is a supporting artifact of the project itself. It lives under the project's `resources/` directory (a project-mode path outside `resources/` fails `validate-task-readiness`). The assignee returns the document inline in its completion report and the PM persists it with `cartopian write-resource`, because the assignee is not granted write access inside the project. The protocol default is `project:resources/<kind>/<title-slug>.md` (kind lower-cased, title slug with identifiers removed): omit the line or write `Deliverable: default` on a `DESIGN` or `RESEARCH` task and `cartopian write-task` stamps it. An explicit path is the override; the operator is asked only for a `root:` destination.
...
A|<identity>|<governing-constraint|outside-scope>|<why this task is not governed by it as a criterion>
...
- An `A|` applicability record is the PM's routine scoping decision for one inherited identity (source or excerpt) that no criterion traces to: `governing-constraint` means it constrains how the work is done and is carried in the task body, not as a criterion; `outside-scope` means it is mechanically unrelated to this task's outcome. It needs no operator authority; the reviewer confirms it at D2 and records `request-uncovered` / `source-uncovered` on the task line if it disagrees. `A|` records are inside the hashed trace identity, so adding one after a review invalidates that review. An identity carries at most one `A|` record and never both an `A|` and a `W|`. Coverage stays whole at the plan level: `plan-audit` warns and `close-audit` blocks on an excerpt every task scopes out and no task claims, unless a decision records `Out-of-plan request: sha256:<content identity>`.
- A `W|` waiver is the record for an actual departure from operator intent and requires attributable operator authority for that exact identity and class. Neither the PM nor the assignee may grant one. Do not use a waiver for mechanically unrelated material — that is an `A|` record.
