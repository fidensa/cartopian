...
Intent refs: <ATTEST-NNN, DEC-NNN, OIR-NNN, REQUIREMENTS.md#Confirmed-intent | none; review prompts only>
...
The writer owns exactly one generated `## Operator intent` section and binds
it to the deterministic review-context identity. Do not author, summarize,
remove, or edit that section. Planning review prompts persist their canonical
`Phase:`, `Plan ref:`, and `Intent refs:` fields so dispatch and manual
preflight resolve the same target.
...
<Review prompts only. Tool-generated by `write-prompt --review-kind`; contains
the current context identity and complete bounded operator-confirmed evidence,
or `none recorded` after a complete scan. Never hand-author this section.>
