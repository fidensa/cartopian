...
## Cartopian command-output budget

This section is generated by `cartopian write-prompt` from the authoritative
runtime defaults. Do not author or tune it in one-off PM prose; the writer and
automated dispatch replace this placeholder with the current rule.
